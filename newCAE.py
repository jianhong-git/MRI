...
def autoencoder(nx, ny, nz, n_input):
    ...
    h_conv = tf.nn.relu(tf.add(tf.nn.conv3d( x_input, W_conv, strides=[1, 1, 1, 1, 1], padding='SAME'), b_conv))
    ...
    y = tf.nn.relu(tf.add(tf.nn.conv3d_transpose(h_conv, W_deconv, tf.shape(x_input), strides=[1, 1, 1, 1, 1], padding='SAME'), b_deconv))
...
def fully_connected(num_pix, num_classes):
    ...
    W_fc1 = tf.Variable(tf.truncated_normal([num_pix, 2000], stddev=0.1), name='fc1_weights')
    b_fc1 = tf.Variable(tf.truncated_normal([2000], stddev=0.1), name='fc1_biases')
    W_fc2 = tf.Variable(tf.truncated_normal([2000, 500], stddev=0.1), name='fc2_weights')
    b_fc2 = tf.Variable(tf.truncated_normal([500], stddev=0.1), name='fc2_biases')
    W_fc3 = tf.Variable(tf.truncated_normal([500, num_classes], stddev=0.1), name='fc3_weights')
    b_fc3 = tf.Variable(tf.truncated_normal([num_classes], stddev=0.1), name='fc3_biases')
    h_fc1 = tf.nn.relu(tf.matmul(x, W_fc1) + b_fc1)
    h_fc2 = tf.nn.relu(tf.matmul(h_fc1, W_fc2)+b_fc2)
    logits = tf.matmul(h_fc2, W_fc3)+b_fc3
    loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits, name='xentropy'), name="loss")
    regularizer = tf.nn.l2_loss(W_fc1) + tf.nn.l2_loss(b_fc1) + tf.nn.l2_loss(W_fc2) + tf.nn.l2_loss(b_fc2) + tf.nn.l2_loss(W_fc3) + tf.nn.l2_loss(b_fc3)
...
#     image_data = np.nan_to_num((image_data-np.nanmin(image_data))/(np.nanmax(image_data)-np.nanmin(image_data)))
    ...
    for index in range(num_batches):
        ...
        for filename, label in np.concatenate((data_batch,hos_data_train)):
            ...
            if (step+1) % display_step == 0:
                print("step",step+1, "in epoch", epoch_i, "loss:", sess.run(ae1['cost'], feed_dict={ae1['x']: data}))
            step += 1
...
for epoch_i in range(epochs):
    ...
    for index in range(num_batches):
        ...
        for filename, label in np.concatenate((data_batch,hos_data_train)):
            ...
            if (step+1) % display_step == 0:
                print("step",step+1, "in epoch", epoch_i, "loss:", sess.run(ae2['cost'], feed_dict={ae2['x']: input_data}))
            step += 1
...
for epoch_i in range(epochs):
    ...
    for index in range(num_batches):
        ...
        for filename, label in np.concatenate((data_batch,hos_data_train)):
            ...
            if (step+1) % display_step == 0:
                print("step",step+1, "in epoch", epoch_i, "loss:", sess.run(ae3['cost'], feed_dict={ae3['x']: input_data}))
            step += 1
