...
df=np.genfromtxt('vb6_food.csv',delimiter=',')
...
x=torch.from_numpy(a).float().cuda()
y=torch.from_numpy(b).float().cuda()
...
optimizer=torch.optim.Adam(model.parameters(),lr=learning_rate,weight_decay=100)
R2_besttest = 0
R2_besttrain = 0
y_best_pred = 0.0
k=0
