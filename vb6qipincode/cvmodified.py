...
def main():
    ...
    k = 0
    p = 0.8
    R2_test = np.zeros(7 * 10**4)
    R2_val = np.zeros(7 * 10**4)
    Loss = np.zeros(7 * 10**4)
    for t in range(7 * 10**4):  # 10**5
        if t == 1 * 10**4:
            learning_rate = 5e-5
            optimizer = torch.optim.Adam(
                model.parameters(), lr=learning_rate, weight_decay=1)
        ...
        y_pred_test = model(Variable(x_test))
        loss_test = loss_fn(y_pred_test.view(-1), Variable(y_test))
        # R2_besttest = (1 - (loss_test.data[0] / den_test))
        if (t + 1) % 1 == 0:
            # Compute and print loss.
            y_pred_val = model(Variable(x_val))
            # Compute and print loss.
            loss_val = loss_fn(y_pred_val.view(-1), Variable(y_val))
            R2_test[t] = 1 - (loss_test.data[0] / den_test)
            R2_val[t] = 1 - (loss_val.data[0] / den_test)
            Loss[t] = loss.data[0]
        ...
        if (t + 1) % 100 == 0:
            ...
            print ('train R^2 : ', (1 - (loss.data[0] / den)))
            # print ('val loss=',
